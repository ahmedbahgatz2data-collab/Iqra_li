...
export class SavioStudioService {
  private ai = new GoogleGenAI({ apiKey: (process.env.API_KEY || process.env.GEMINI_API_KEY || '') as string });

  async extractTextFromImage(base64Data: string, mimeType: string = 'image/jpeg'): Promise<string> {
    const prompt = `أنت خبير قراءة واستخراج النصوص والمخطوطات العربية في استوديو "سافيو VO".
مهمتك: قراءة الصورة واستخراج كافة النصوص العربية المكتوبة فيها بدقة متناهية وإتقان تام (سواء كانت نصوصاً مطبوعة، صفحات كتب، لافتات، منشورات، إعلانات، أو وثائق).

التعليمات:
1. استخرج النص كما هو بدقة مع ترتيب الفقرات والأسطر بشكل منطقي.
2. حافظ على علامات الترقيم والتشكيل إن وجد.
3. لا تضف أي تعليقات، اعتذارات، أو مقدمات مثل "إليك النص المستخرج". أخرج النص المكتوب فقط.`;
